...
Options:
  ...
  --gold PATH        gold JSONL (default: inputs/gold.jsonl)
...
INPUTS      = REGEVAL_DIR / "inputs"
EXPERIMENTS = REGEVAL_DIR / "experiments"
LOG_PATH    = EXPERIMENTS / "log.md"
TSV_PATH    = REGEVAL_DIR / "results.tsv"
QUEUE_PATH  = REGEVAL_DIR / "review-queue.md"

QUEUE_MARKER = "<!-- RUN-BLOCKS (newest first; do not edit above this line) -->"
QUEUE_HEADER = f"""# RegEval — human review queue

Auto-generated by `score.py` after each run. Two slices land here, because a
high-stakes judge fails in two directions and error analysis must see both:

- **⏳ Abstains** — the judge said `unclear`. `GENUINE` = gold is also unclear
  (correct escalation). `HEDGE` = gold was decidable (judge over-abstained → a
  scaffold-refine signal).
- **⚠ Confident mismatches** — the judge **committed** to a class and was **wrong**.
  This is the catastrophic slice the abstain queue is blind to. `FALSE-CLEAR` =
  judge cleared an actual violation (worst case for a compliance judge). `OVER-COMMIT`
  = judge committed on a genuinely-unclear item. `FALSE-FLAG` = judge flagged
  compliant content. These are scaffold-defect signals, not gold-error signals.
...
# --- review queue (abstain → human routing) ---------------------------------

def _excerpt(text, n=90):
    return re.sub(r"\s+", " ", (text or "")).strip()[:n]

def _confident_mismatch_kind(gold, pred):
    """Classify a committed (non-abstain) wrong verdict by danger, worst first.

    With a labels-only harness, 'confident' = the judge committed to a definite
    class rather than abstaining. The direction of the error is what matters for
    a compliance judge: clearing an actual violation is the catastrophic case.
    """
    if pred == "compliant" and gold == "non-compliant":
        return (0, "FALSE-CLEAR — judge cleared an actual violation (CATASTROPHIC)")
    if gold == "unclear":
        return (1, "OVER-COMMIT — judge committed on a genuinely-unclear item")
    if pred == "non-compliant" and gold == "compliant":
        return (2, "FALSE-FLAG — judge flagged compliant content")
    return (3, "MISMATCH — committed verdict disagrees with gold")

def write_review_queue(run_id, exp_name, kappa, abst_v, ids, gold, pred,
                       gold_input_by_id, gold_path, harness):
    """Prepend this run's human-review items to review-queue.md — two slices:

    - abstains (pred == unclear): the judge hedged.
    - confident mismatches (pred != gold and pred != unclear): the judge committed
      and was wrong — the catastrophic slice the abstain queue is blind to.

    Each item is a checkbox line that `judge_vs_human.py adjudicate --log` later
    flips from ⏳/⚠ to ✅. Newest run on top; prior blocks are preserved untouched.
    """
    abstains = [(i, g) for i, g, p in zip(ids, gold, pred) if p == "unclear"]
    genuine = sum(1 for _, g in abstains if g == "unclear")
    hedge = len(abstains) - genuine

    # confident-and-wrong: judge committed to a class but disagreed with gold
    confident = sorted(
        ((i, g, p) for i, g, p in zip(ids, gold, pred) if p != "unclear" and g != p),
        key=lambda t: _confident_mismatch_kind(t[1], t[2])[0],
    )
    false_clear = sum(1 for _, g, p in confident if p == "compliant" and g == "non-compliant")

    def _cmd(i):
        return (f"  `echo <your-label> | python3 judge_vs_human.py adjudicate "
                f"experiments/{exp_name} {i} --gold-snapshot {gold_path} --log`")

    ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%MZ")
    lines = [
        f"## {run_id} · κ={kappa:.3f} · abstention={abst_v:.3f} · "
        f"{len(abstains)} abstains ({genuine} genuine / {hedge} hedge) · "
        f"{len(confident)} confident mismatches ({false_clear} false-clear) · generated {ts}",
    ]
    if harness == "subagent":
        lines.append("_Subagent-harness run: judge emitted labels only — no per-item "
                     "rationale/trigger available (those exist on api runs)._")

    lines.append("")
    lines.append("### ⏳ Abstains (judge hedged)")
    if not abstains:
        lines.append("_No abstains in this run._")
    for i, g in abstains:
        kind = "GENUINE — human review warranted" if g == "unclear" else "HEDGE — judge over-abstained"
        lines.append(f'- [ ] **{i}** · gold=`{g}` ({kind}) · "{_excerpt(gold_input_by_id.get(i, ""))}…"')
        lines.append(_cmd(i))

    lines.append("")
    lines.append("### ⚠ Confident mismatches (judge committed and was wrong)")
    if not confident:
        lines.append("_No confident mismatches in this run._")
    for i, g, p in confident:
        kind = _confident_mismatch_kind(g, p)[1]
        lines.append(f'- [ ] **{i}** · gold=`{g}` judge=`{p}` ({kind}) · '
                     f'"{_excerpt(gold_input_by_id.get(i, ""))}…"')
        lines.append(_cmd(i))
    block = "\n".join(lines) + "\n"

    if QUEUE_PATH.exists():
        existing = QUEUE_PATH.read_text()
        if QUEUE_MARKER in existing:
            head, tail = existing.split(QUEUE_MARKER, 1)
            QUEUE_PATH.write_text(head + QUEUE_MARKER + "\n\n" + block + tail.lstrip("\n"))
        else:  # malformed/legacy file — rebuild header, keep old content below
            QUEUE_PATH.write_text(QUEUE_HEADER + "\n" + block + "\n" + existing)
    else:
        QUEUE_PATH.write_text(QUEUE_HEADER + "\n" + block)
    return len(abstains), len(confident)
...
def main():
    ...
    ap.add_argument("--gold", default=str(INPUTS / "gold.jsonl"))
    ...
    ap.add_argument("--no-queue", action="store_true",
                    help="do not write/refresh review-queue.md (default: write it)")
    args = ap.parse_args()
    ...
    gold_input_by_id = {g["id"]: g.get("input", "") for g in gold_items}
...
""")
    ...
    # 4) review-queue.md — route this run's abstains to a human (unless suppressed)
    if not args.no_queue:
        run_id = f"{date_str}-{nn}-{args.slug}"
        n_abst, n_conf = write_review_queue(run_id, exp.name, kappa, abst_v, ids, gold, pred,
                                            gold_input_by_id, args.gold, args.harness)
        print(f"Refreshed review-queue.md → {n_abst} abstain(s) + "
              f"{n_conf} confident mismatch(es) queued for human adjudication")
